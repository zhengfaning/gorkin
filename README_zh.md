...
在您的项目中，您可以通过 `npx @zhengfaning/gorkin [command]` 操控工具的完整生命周期：

| 命令 | 描述 |
| :--- | :--- |
| `npx @zhengfaning/gorkin init` | 智能生成配置模板与演示代码（自带防覆盖安全锁） |
| `npx @zhengfaning/gorkin dev` | 启动本地开发服务器。支持 Markdown 热重载，写文档时实时预览 |
| `npx @zhengfaning/gorkin build`| 构建生产级静态站点（生成的文件位于 `.vitepress/dist`） |
| `npx @zhengfaning/gorkin serve`| 在本地预览构建好的生产环境静态站点 |
| `npx @zhengfaning/gorkin test` | 脱机测试指令。仅扫描 Go 模块并执行全量测试，生成关联报告 |
